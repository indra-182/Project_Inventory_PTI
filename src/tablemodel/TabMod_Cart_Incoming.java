 ...
 * @author rafii
 */
public class TabMod_Cart_Incoming extends AbstractTableModel {
private List<Model_Cart_Incoming> list = new ArrayList();
public void addData(Model_Cart_Incoming mci){
        list.add(mci);
        fireTableRowsInserted(list.size()-1,list.size()-1);
        JOptionPane.showMessageDialog(null,"Data has been Added");
    }
    public void updateData(int row, Model_Cart_Incoming mci){
        list.add(row ,mci);
    ...
    public void setData(List<Model_Cart_Incoming>list){
        clear();
        this.list.addAll(list);
        fireTableDataChanged();
    }
    
    public void setData(int index, Model_Cart_Incoming mci){
        list.set(index,mci);
        fireTableRowsUpdated(index,index);
    }
    
    public Model_Cart_Incoming getData(int index){
    ...
    public Object getValueAt(int rowIndex, int columnIndex) {
        switch(columnIndex){
            case 0: return list.get(rowIndex).getModi().getMoin().getId_incoming();
            ...
            case 5: return list.get(rowIndex).getModi().getQuantity();
            ...
            case 9: return list.get(rowIndex).getModi().getTotal_price();
            default:return null;
        }
    }
